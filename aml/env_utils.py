...
def aux_from_infos(infos, device):
    ...
    aux = []
    for info in infos:
        aux.append(torch.from_numpy(info['aux']))
...
def make_vec_envs(env_name, env_args, seed, num_envs, device):
    if env_name.startswith('BlockOnIncline'):
        num_envs_types = 6  # we have 6 types of rearrange environments
        ...
        for i in range(num_envs):
            ...
            if any_env_skin > 1:
                v += (any_env_skin-1)*num_envs_types
